...
app = Bottle()
names = ["bulb1", "bulb2"]
ips = ["192.168.100.10", "192.168.100.11"]
...
def route_handler(func):
    ...
    def wrapper():
        ...
        try:
            ...
            pass
        if params is None:
            params = {p: getattr(request.params, p) for p in sig.parameters}
        print(params)
        result = func(**params)
...
def get_device_state():
    if connected is None:
        return False
    return {
        'name': names[connected],
        'ip': ips[connected],
        'color': colors[connected],
        'brightness': brights[connected]
    }


@app.route('/brightness', method='POST')
@route_handler
def set_device_brightness(brightness):
    if connected is None:
        return False
    brightness = float(brightness)
    if 0 <= brightness < 10.0:
        brights[connected] = brightness
    return brights[connected] == brightness


@app.route('/color', method='POST')
@route_handler
def set_device_color(color):
    if connected is None:
        return False
    try:
        hex_to_rgb(color)
...
def set_device_name(name):
    if connected is None:
        return False
    names[connected] = name
    return True


@app.route('/chilltime', method='POST')
@route_handler
def chill_time():
    if connected is None:
        return False
    if brights[connected] > 0:
        if brights[connected] > 3.0:
            brights[connected] *= 0.5
        else:
            brights[connected] += 1.0
    else:
        return False
    return True
